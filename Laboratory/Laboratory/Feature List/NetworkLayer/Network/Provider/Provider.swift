    ...
    func request<R: Decodable, E: RequesteResponsable>(with endpoint: E, completion: @escaping (Result<R, Error>) -> Void) where E.Response == R {
        do {
            ...
            session.dataTask(with: urlRequest) { [weak self] data, response, error in
                print("step 1")
                let errorchecker = ErrorChecker()
                errorchecker.excute(with: data, response, error) { result in
                    switch result {
                    case .success(let data):
                        completion(errorchecker.decode(data: data))
...
struct ErrorChecker {
    ...
    func excute(with data: Data?, _ response: URLResponse?, _ error: Error?, completion: @escaping (Result<Data, Error>) -> ()) {
        print(#function)
        if let error = error {
            completion(.failure(error))
            return
